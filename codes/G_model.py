...
class G(nn.Module):
    def __init__(self, dim_ent, hid_dim_ratio_ent, dim_rel, hid_dim_ratio_rel, num_bin, num_layer_ent=2, num_layer_rel=2, \
        ...
        for _ in range(num_layer_ent):
            layers_ent.append(GEntityLayer(layer_dim_ent, layer_dim_ent, layer_dim_rel, \
                                                bias = bias, num_head = num_head))
        for _ in range(num_layer_rel):
            layers_rel.append(GRelationLayer(layer_dim_rel, layer_dim_rel, num_bin, \
                                                  bias = bias, num_head = num_head))
        res_proj_ent = []
        for _ in range(num_layer_ent):
